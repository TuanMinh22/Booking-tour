...
export default function TourPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get("/tours?limit=20");
  ...
  return (
    <>
      ...
      <Container>
        ...
        <Card>
          <UserListToolbar numSelected={selected.length} filterName={filterName} onFilterName={handleFilterByName} searchFor='Tìm kiếm theo Tên tour' />
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => {
                    const { TourId, nameTour, Occupany, tourPrice, dateStart, dateEnd, time, include, uninclude, urlImg, des } = row;
                    const selectedUser = selected.indexOf(nameTour) !== -1;

                    return (
                      <TableRow hover key={TourId} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        <TableCell padding="checkbox">
                          <Checkbox checked={selectedUser} onChange={(event) => handleClick(event, nameTour)} />
                        </TableCell>
                        <TableCell align="left" sx={{ maxWidth: 150 }}>{nameTour}</TableCell>
                            ...
                            <div className="dialog-overlay" >
                              <div className="dialog-content" >
                                ...
                                <div className="dialog-buttons">
                                  ...
                                  <button className="dialog-button dialog-button-confirm" onClick={() => handleDeleteConfirm(index, row.Tour_ID)}>Có</button>
                                </div>
                              </div>
                            </div>
                          )}
