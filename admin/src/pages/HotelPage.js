...
const TABLE_HEAD = [
  { id: 'HotelName', label: 'Tên khách sạn', alignRight: false },
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(array, (_user) => _user.HotelName.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
...
export default function HotelPage() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = info.map((n) => n.HotelName);
    ...
    <>
      ...
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          ...
          <Link to='/dashboard/hotel/create'>
        ...
        <Card>
          <UserListToolbar numSelected={selected.length} filterName={filterName} onFilterName={handleFilterByName} searchFor='Tìm kiếm theo Tên khách sạn' />
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => {
                    const { HotelId, HotelName, Phone, Address, img, name } = row;
                    const selectedUser = selected.indexOf(HotelName) !== -1;

                    return (
                      <TableRow hover key={HotelId} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        <TableCell padding="checkbox">
                          <Checkbox checked={selectedUser} onChange={(event) => handleClick(event, HotelName)} />
                        </TableCell>
                        ...
                        <TableCell component="th" scope="row" padding="none">
                          <Stack direction="row" spacing={1}>
                            <Avatar alt={HotelName} src={img} />
                          </Stack>
                        </TableCell>

                        <TableCell align="left">{Phone}</TableCell>

                        <TableCell align="right" key={HotelId}>
                          <Link to={`/dashboard/hotel/${row.Hotel_ID}`}>
                            ...
                            <div className="dialog-overlay" >
                              <div className="dialog-content" >
                                ...
                                <div className="dialog-buttons">
                                  ...
                                  <button className="dialog-button dialog-button-confirm" onClick={() => handleDeleteConfirm(index, row.Hotel_ID)}>Có</button>
                                </div>
                              </div>
                            </div>
                          )}
